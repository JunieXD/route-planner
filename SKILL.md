...
3. 跨城铁路先运行 `scripts/rail_12306.py direct`。若没有直达或换乘可能更符合偏好，再运行 `transfer`：它先做有界枢纽搜索，再从已找到的列车对读取完整经停站、发现共同中间站并重新查询分段票价与余票。读取 `search_coverage` 中的枢纽、经停站细化和预算截断统计；覆盖不完整时只能写“已查最省/最快”，不能写“全网最低/最快”。若用户给出具体车站，保留该车站；若只给城市，应比较该城市实际可用的多个到发站。
4. 保留价格、时长和到达站有差异的铁路候选，为每个候选补齐首末段后再做最终排名。铁路段最省不等于门到门最省：
...
6. 途经点按用户给定顺序拆成连续路段。将换乘步行、候车、铁路进站预留和用户要求的停留时间分别计入，避免只相加车内时间。若用户给出最晚到达时间，应倒推最晚安全出门时间并把到达期限作为硬约束；若截止时间缺失且会改变推荐，给出条件结论或只询问这一项关键约束。
...
- 组合方案交给 `rank_routes.py` 后检查其强制生成的 `door_to_door_duration_minutes`、`scheduled_span_minutes`、`in_vehicle_minutes`、`waiting_minutes`、`checkin_buffer_minutes` 和 `local_connection_minutes`。起点接驳未知时，门到门耗时必须保持不完整，改写为“从提前到达首站开始的已知耗时 + 住处接驳”；不能把单段车程或铁路跨度写成整体耗时。
...
- 清晨和深夜接续必须检查 `service_window_verified`。提供方没有首末班数据时，精确到达值只能写成“若该时段可乘，最早约 HH:MM”，并标“首末班待确认”；不要从普通路线耗时推断一定可乘。

## 用户通常还会在意

- 接驳和换乘是否现实：站内外步行时间、步行距离、跨站换乘、最紧换乘余量，以及错过固定班次后的影响。
- 方案是否稳妥：余票或开售状态、数据新鲜度、公交班次稀疏、首末班车限制、动态票价和未计费用。连接可靠性与出行负担分开表达：长候车通常提高接续可靠性，却会增加负担。只对有证据的差异标“稳 / 一般 / 赶 / 待确认”。
...
- 可执行性：默认把当前可售方案与仅报价/售罄预览分开。只有 `inventory_confirmed=true` 的方案可获得推荐、最快或最省标签；售罄卧铺可展示升级报价，但不能伪装成可执行选择。
...
- 不因两个车站属于同一地级行政区就假定跨站换乘可行。`--allow-cross-station` 仍须同时提供 `--cross-station-rules`，其中每条接驳都含已验证的耗时、费用和来源；没有真实接驳数据时排除候选。
