...
def read_routes(path: Path) -> list[dict[str, Any]]:
    ...
    if not isinstance(payload, list):
        raise ValueError("input JSON must be a route array or an object containing a routes array")
...
def filter_reasons(route: dict[str, Any], args: argparse.Namespace) -> list[str]:
    ...
    if args.max_duration_minutes is not None and duration > args.max_duration_minutes:
        reasons.append("exceeds_max_duration")
    if args.max_transfers is not None and transfers > args.max_transfers:
...
def add_balanced_scores(routes: list[dict[str, Any]]) -> None:
    ...
    known_prices = [float(route["price_cny_per_person"]) for route in routes if route["price_cny_per_person"] is not None]
    for route in routes:
        price = route["price_cny_per_person"]
        price_score = normalized(float(price), known_prices) if price is not None and known_prices else 1.1
        score = (
...
def dominates(left: dict[str, Any], right: dict[str, Any]) -> bool:
    if left["price_cny_per_person"] is None or right["price_cny_per_person"] is None:
        return False
    left_values = (
...
def pareto_ids(routes: list[dict[str, Any]]) -> set[str]:
    priced = [route for route in routes if route["price_cny_per_person"] is not None]
    return {
        str(candidate["id"])
        for candidate in priced
        if not any(dominates(other, candidate) for other in priced if other is not candidate)
    }
...
def sort_key(route: dict[str, Any], optimize: str) -> tuple[Any, ...]:
    ...
    price_key = (price is None, price if price is not None else math.inf)
    if optimize == "fastest":
        return duration, transfers, risk, *price_key
    if optimize == "cheapest":
        return *price_key, duration, transfers, risk
    if optimize == "fewest-transfers":
        return transfers, duration, risk, *price_key
...
def label_routes(routes: list[dict[str, Any]], pareto: set[str], optimize: str) -> None:
    min_duration = min(float(route["duration_minutes"]) for route in routes)
    min_transfers = min(int(route["transfer_count"]) for route in routes)
    priced = [route for route in routes if route["price_cny_per_person"] is not None]
    min_price = min(float(route["price_cny_per_person"]) for route in priced) if priced else None
    for rank, route in enumerate(routes, start=1):
        labels: list[str] = []
        if float(route["duration_minutes"]) == min_duration:
            labels.append("fastest")
        if min_price is not None and route["price_cny_per_person"] == min_price:
            labels.append("cheapest")
        if int(route["transfer_count"]) == min_transfers:
        ...
        route["ranking"] = {
            ...
            "labels": labels,
            "price_complete": route["price_cny_per_person"] is not None,
        }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("input", type=Path, help="JSON file matching references/route-schema.md")
...
def main() -> int:
    ...
    try:
        ...
        for flag in ("max_duration_minutes", "max_price_per_person", "max_total_price", "max_transfers"):
        ...
        for route in source_routes:
            price = route["price_cny_per_person"]
            route["price_cny_total"] = None if price is None else round(price * args.party_size, 2)
